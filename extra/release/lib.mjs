...
export function getRepoNames() {
    ...
    return ["louislam/uptime-kuma-revanced", "ghcr.io/louislam/uptime-kuma-revanced"];
}
 ...
 * docker buildx build -f docker/dockerfile --platform linux/amd64 -t louislam/uptime-kuma-revanced:upload-artifact --build-arg VERSION --build-arg GITHUB_TOKEN --target upload-artifact . --progress plain
 * @param {string} version Version
...
export function uploadArtifacts(version, githubToken) {
    let args = [
        ...
        "linux/amd64",
        "-t",
        "louislam/uptime-kuma-revanced:upload-artifact",
        "--build-arg",
        `VERSION=${version}`,
        "--build-arg",
        "GITHUB_TOKEN",
...
export async function createReleasePR(version, previousVersion, dryRun, branchName = "release", githubRunId = null) {
    ...
    const artifactLink = githubRunId 
        ? `https://github.com/louislam/uptime-kuma-revanced/actions/runs/${githubRunId}/workflow`
        : `https://github.com/louislam/uptime-kuma-revanced/actions/workflows/beta-release.yml`;
