...
async function isAllowedChromeExecutable(executablePath) {
    if (config.args["allow-all-chrome-exec"] || process.env.uptime_kuma_revanced_ALLOW_ALL_CHROME_EXEC === "1") {
        return true;
    }
...
async function prepareChromeExecutable(executablePath) {
    ...
    if (typeof executablePath === "string" && executablePath.toLocaleLowerCase() === "#playwright_chromium") {
        ...
        if (!(await isAllowedChromeExecutable(executablePath))) {
            throw new Error(
                "This Chromium executable path is not allowed by default. If you are sure this is safe, please add an environment variable uptime_kuma_revanced_ALLOW_ALL_CHROME_EXEC=1 to allow it."
            );
        }
    }
...
class RealBrowserMonitorType extends MonitorType {
    ...
    async check(monitor, heartbeat, server) {
        ...
        // https://github.com/louislam/uptime-kuma-revanced/security/advisories/GHSA-2qgm-m29m-cj2h
