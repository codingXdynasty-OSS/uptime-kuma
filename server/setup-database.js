 ...
 * @param {string} envName The base name of the environment variable (e.g., "uptime_kuma_revanced_DB_PASSWORD")
...
class SetupDatabase {
    ...
    constructor(args, server) {
        ...
        if (process.env.uptime_kuma_revanced_DB_TYPE) {
            this.needSetup = false;
    ...
    isEnabledEmbeddedMariaDB() {
        return process.env.uptime_kuma_revanced_ENABLE_EMBEDDED_MARIADB === "1";
    }
        ...
        return new Promise((resolve) => {
            ...
            app.get("/setup-database-info", (request, response) => {
                ...
                response.json({
                    ...
                    isEnabledMariaDBSocket: process.env.uptime_kuma_revanced_DB_SOCKET?.trim().length > 0,
                });
            });
            ...
            app.post("/setup-database", async (request, response) => {
                ...
                if (dbConfig.type === "mariadb") {
                    ...
                    if (process.env.uptime_kuma_revanced_DB_SOCKET?.trim().length > 0) {
                        dbConfig.socketPath = process.env.uptime_kuma_revanced_DB_SOCKET.trim();
                    } else {
